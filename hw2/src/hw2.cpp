...
void leadertoworld2D(geometry_msgs::Point &follower_goal, turtlesim::Pose &leader)
{
	...
	follower_goal.y = cos(-leader.theta)*temp_y-sin(-leader.theta)*temp_x+leader.y;
...
void worldtobody2D(float &x, float &y, float theta)
{
	...
	float numx=x;
	float numy=y;
	x=cos(theta)*numx+sin(theta)*numy;
	y=cos(theta)*numy-sin(theta)*numx;
...
void Positioncontrol(geometry_msgs::Point &goal, turtlesim::Pose &follower, geometry_msgs::Twist &vel_msg) {
	...
	vel_msg.linear.x = Kpx*error_norm;
	vel_msg.angular.z = Kpz*error_theta;
